...
class Encoder_Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.encoder = DenseNet(growthRate=params['growthRate'], reduction=params['reduction'],
                                bottleneck=params['bottleneck'], use_dropout=params['use_dropout'], img_channel=params['input_channels'])
    ...
    def encoder_forward(self, x, x_mask):
        ctx, ctx_mask = self.encoder(x, x_mask)
        torch.cuda.empty_cache()
        return ctx, ctx_mask

    def decoder_forward(self, ctx, ctx_mask, params, y, y_mask, one_step):
        # init state
        ...
        h2ts, cts, alphas, _alpha_pasts = self.gru_model(params, emb, y_mask, ctx, ctx_mask, one_step, init_state, alpha_past=None)
    ...
    def forward(self, params, x, x_mask, y, y_mask, one_step=False):
        ...
        ctx, ctx_mask = self.encoder_forward(x, x_mask)
        
        alphas, scores = self.decoder_forward(ctx, ctx_mask, params, y, y_mask, one_step)
